...
class EnemyNode: SKSpriteNode {
    ...
    required init?(coder aDecoder: NSCoder) {
        fatalError("Impossible to get this error IMO")
    }
    ...
    func configureMovement(_ moveStraight: Bool) {
        ...
        if moveStraight {
            ...
            path.addCurve(to: CGPoint(x: -3500, y: 0),  controlPoint1: CGPoint(x: 0, y: -position.y * 4),
                                                        controlPoint2: CGPoint(x: -1000, y: -position.y))
        }
